...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    sliderList.clear();
    if (widget.preQty == null || widget.preQty == "") {
      newCounter = 1;
    } else {
      newCounter = int.parse(widget.preQty.toString());
    }
...
/*  _transactionscrollListener() {
  ...
  Widget build(BuildContext context) {
    print("checking seller id here now ${widget.sellerId}");
  ...
  showBottom(Product model) async {
                                                            ...
                                                            persistentBottomSheetController!
                                                              ...
                                                              if (addIdList.indexWhere((element) =>
                                                                      ...
                                                                      model
                                                                          ...
                                                                          .id!) !=
                                                                  -1) {
                                                                addIdList[addIdList.indexWhere((element) =>
                                                                        element
                                                                            .id ==
                                                                        model
                                                                            .addOnList![i]
                                                                            .id!)]
                                                                    .qty = model.addOnList![i].cartCount!;
                                                              }
                                                              priceAdd += double
                                                                  .parse(model
                                                                      .addOnList![
...
*/
  ...
  indicatorImage() {
    ...
    print("checking final indicator here ${indicator}");
                          ...
                          children: <Widget>[
                            ...
                            GestureDetector(
                              ...
                              onTap: () {
                                ...
                                //   //   showBottom(widget.model!);
                                //   //   return;
                                //   // }
                                //   // if (widget.model!.is == "0") {
                                ...
                                //       return;
                                //     }
                                //     addToCart(qty, true, addIdList);
                                //   //}
                                //  // else {
                                //     // if (aadharImage == null) {
                                //     //   setSnackbar("Please Select Image", context);
                                //     // }
                                //
                                //       // String qty = ((int.parse(qtyController.text)) + 0
                                //       //     // (int.parse(widget.model!.qtyStepSize!))
  ...
  void _chooseVarient() {
    ...
    print(
        "data here now value ${widget.model!.stockType} and ${widget.model!.availability} and ${widget.model!.selVarient}");
    if (widget.model!.stockType == "0" || widget.model!.stockType == "1") {
      if (widget.model!.availability == "1") {
        print("kokooookkooo ${widget.model!.selVarient}");
        setState(() {
          available = true;
          outOfStock = false;
          _oldSelVarient = widget.model!.selVarient;
  ...
  applyVarient() {
    Navigator.of(context).pop();
    print("checking variant here ${_oldSelVarient}");
    if (mounted)
      setState(() {
  ...
  Future<void> addToCart(
      String qty, bool intent, List<AddQtyModel> addList) async {
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      if (CUR_USERID != null) {
        try {
          ...
          if (int.parse(qty) > 1) {
            index1 = cartList.indexWhere((element) =>
                element.varientId ==
                widget.model!.prVarientList![model.selVarient!].id);
          }
          List<String> idList = [];
          List<String> qtyList = [];
          ...
          var parameter = {
            ...
            QTY: qty,
            "add_on_id": addIdList.isEmpty
                ? ""
                : index1 != -1 && int.parse(qty) > 1
